...
var InitSystem = function()
{
	...
	function startShutdownByError()
	{
		...
		finishWithError('Error while shutting down');
	}
	...
	function finishWithError(error)
	{
		if (self.options.showErrors)
		{
			self.log.error('Error while finishing in phase %s: %s', phase, error);
		}
	...
	self.shutdown = function(errorCode)
	{
		...
		runAll('stop', function(error)
		{
			...
			runAll('finish', function(error)
			{
				if (error)
				{
					return finishWithError('Could not run finish tasks: ' + error);
				}
				if (self.options.logTimes)
				{
					var elapsedSeconds = (Date.now() - start) / 1000;
