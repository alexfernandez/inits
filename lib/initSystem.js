...
var InitSystem = function()
{
	...
	function runAll(event, callback)
	{
		...
		runner(taskQueues[event], function(error)
		{
			if (error)
			{
	...
	function runInParallel(queue, callback)
	{
		if (!queue.length)
		{
			return callback(null);
	...
	function setupErrors()
	{
		if (self.options.catchErrors)
		{
			...
			{
				if (!finished && self.options.showErrors)
				{
