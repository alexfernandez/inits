...
InitSystem.prototype.getAdder = function(event)
{
	...
	return function(task)
	{
		...
		self.taskQueues[event].push(task);
		return {
			before: function(dependent)
			{
				self.constraints.push(self.createConstraint(event, task, dependent, 0));
			},
			after: function(dependent)
			{
				self.constraints.push(self.createConstraint(event, task, dependent, +1));
			},
		};
	};
};

InitSystem.prototype.createConstraint = function(event, task, dependent, offset)
{
	var self = this;
	return function()
	{
		self.taskQueues[event].remove(task);
		var index = self.taskQueues[event].indexOf(dependent);
		if (index == -1)
		{
			if (self.options.showErrors)
			{
				self.log.warning('Could not find task %s', dependent.name);
				self.taskQueues[event].push(task);
				return;
			}
		}
		self.log.debug('Inserting %s at %s (%s) + %s', task.name, dependent.name, index, offset);
		self.taskQueues[event].splice(index + offset, 0, task);
	}
}

InitSystem.prototype.runConstraints = function()
{
...
InitSystem.prototype.runAll = function(event, callback)
{
	...
	if (self.taskQueues[event].length)
	{
		self.log.debug('Running queue for %s: %s tasks', event, self.taskQueues[event].length);
	}
	self.phase = event;
...
InitSystem.prototype.runInParallel = function(queue, callback)
{
	...
	{
		self.run(queue[i], function(error)
		{
			if (error)
			{
				if (self.options.showErrors)
				...
				if (self.options.stopOnError && received < queue.length)
				{
...
InitSystem.prototype.setupErrors = function()
{
	...
	if (self.options.catchErrors)
	{
		...
		{
			self.manageUnexpectedError('error', error);
		});
		process.on('uncaughtException', function(error)
		{
			self.manageUnexpectedError('uncaught exception', error);
		});
...
InitSystem.prototype.manageUnexpectedError = function(type, error)
{
	var self = this;
	if (!self.options.catchErrors)
	{
	...
	self.startShutdownByError(util.format('Unexpected %s: %s', type, error));
};
